...
def initialize_mlp(sizes, key, affine=[False], scale=1.0):
    ...
    def initialize_layer(m, n, key, affine=True, scale=1.0):
        w_key, b_key = random.split(key)
        if affine:
            return scale * jnp.sqrt(2/m) * random.normal(w_key, (n, m)), 0 * random.normal(b_key, (n,))
        else:
            return scale * jnp.sqrt(2/m) * random.normal(w_key, (n, m)), scale * random.normal(b_key, (n,))
    return [initialize_layer(m, n, k, affine=a, scale=scale) for m, n, k, a in zip(sizes[:-1], sizes[1:], keys, affine)]

def initialize_mlp_gamma(sizes, key, affine=[False], scale=1.0):
    """ Initialize the weights of all layers of a linear layer network """
    keys = random.split(key, len(sizes))
    # Initialize a single layer with Gaussian weights -  helper function
    if len(affine) != len(sizes):
    ...
    def _initialize_layer(m, n, key, affine=True, scale=1.0):
        w_key, b_key = random.split(key)
        if affine:
            return scale * jnp.sqrt(2/m) * random.normal(w_key, (n, m)), 0 * random.normal(b_key, (n,1))
        else:
            return scale * jnp.sqrt(2/m) * random.normal(w_key, (n, m)), scale * random.normal(b_key, (n,1))
    return [_initialize_layer(m, n, k, affine=a, scale=scale) for m, n, k, a in zip(sizes[:-1], sizes[1:], keys, affine)]
...
def Softplus(x):
    return jnp.log(lax.add(1.0, jnp.exp(x)))
...
def forward_pass_gamma(params, x, activation_fn=SquarePlus):
    """ Compute the forward pass for each example individually """
    h = x
...
def GaussianNLL(var, y_act, y_pred, A, B):
    return jnp.mean(A*jnp.log(var)+B*(jnp.square(y_act-y_pred)/var) + 0)
